<x-app-layout>
    ...
    <div class="py-6 max-w-7xl mx-auto">
        ...
        <div class="bg-white p-6 rounded shadow mb-6">
            ...
            <div class="border-b border-gray-200 pb-4 mb-6">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-2xl font-semibold text-gray-900">
                        {{ $notification->data['title'] ?? __('notifications.default_title') }}</h3>
                ...
                <p class="text-sm text-gray-600">{{ __('notifications.created_at') }} <span
            ...
            <!-- Treść powiadomienia -->
            <div class="mb-6">
                <h4 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-2">{{ __('notifications.detail_heading') }}</h4>
                <div
                    class="bg-gray-50 border border-gray-200 rounded p-4 text-gray-800 leading-relaxed whitespace-pre-wrap">
                        {!! nl2br(e(str_replace('\n', "\n", $notification->data['body'] ?? __('notifications.none')))) !!}
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                <h4 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">{{ __('notifications.details') }}</h4>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if(isset($notification->data['device_report_id']))
                    <div class="bg-blue-50 border border-blue-200 rounded p-3">
                        <p class="text-xs font-semibold text-blue-900 uppercase mb-1">{{ __('notifications.report_id') }}</p>
                    ...
                    <div class="bg-purple-50 border border-purple-200 rounded p-3">
                        <p class="text-xs font-semibold text-purple-900 uppercase mb-1">{{ __('notifications.device_id') }}</p>
                    ...
                    <div class="bg-green-50 border border-green-200 rounded p-3">
                        <p class="text-xs font-semibold text-green-900 uppercase mb-1">{{ __('notifications.reporter') }}</p>
                ...
                <div class="mt-4">
                    <p class="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2">{{ __('notifications.full_description') }}</p>
                    <div
        ...
        <div class="flex gap-2">
            <form action="{{ route('notifications.destroy', $notification->id) }}" method="POST" class="inline"
                onsubmit="return confirm(@json(__('notifications.delete_confirm')));">
                @csrf
                @method('DELETE')
