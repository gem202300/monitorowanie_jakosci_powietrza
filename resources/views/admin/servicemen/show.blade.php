    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white text-black shadow-xl sm:rounded-lg p-6">
                ...
                <h4 class="mt-6 font-semibold">{{ __('servicemen.assign_device') }}</h4>
                <form action="{{ route('servicemen.assign', $serviceman) }}" method="POST" class="mt-2">
                    ...
                    <button type="submit" class="mt-2 bg-green-500 text-white px-3 py-1 rounded">{{ __('servicemen.assign') }}</button>
                </form>

                <h4 class="mt-6 font-semibold">{{ __('servicemen.currently_assigned') }}</h4>
                <ul class="mt-2">
                    @foreach($serviceman->devices as $device)
                        <li class="flex justify-between items-center border p-2 rounded mb-1">
                            ...
                            <form action="{{ route('servicemen.unassign', $serviceman) }}" method="POST">
                                ...
                                <button type="submit" class="bg-red-500 text-white px-2 py-1 rounded">{{ __('servicemen.unassign') }}</button>
                            </form>
                        </li>
                    @endforeach
                </ul>
